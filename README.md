...
- **Leo Lang**: `ghcr.io/sealance-io/leo-lang:v4.4.2`
- **Aleo Devnet**: `ghcr.io/sealance-io/aleo-devnet:v4.4.2-v4.9.1`
...
docker run --rm ghcr.io/sealance-io/leo-lang:v4.4.2 leo --help

# Check installed versions
docker run --rm ghcr.io/sealance-io/leo-lang:v4.4.2

# Mount your project directory and work with Leo
docker run --rm -v $(pwd):/app -w /app ghcr.io/sealance-io/leo-lang:v4.4.2 leo build

# Start a shell in the container
docker run --rm -it -v $(pwd):/app -w /app ghcr.io/sealance-io/leo-lang:v4.4.2 /bin/bash
```
...
docker run -it --rm -p 3030:3030 -p 4130:4130 \
  ...
  ghcr.io/sealance-io/aleo-devnet:v4.4.2-v4.9.1
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Setup Leo CLI
        uses: sealance-io/setup-leo-action@7b57645adc0b4c352abf38619dd53f81c2e5fe76 # v1.1.4
        with:
          version: '4.3.1'
          rust-version: '1.96.0'
...
Published image tags stay normalized as `v4.4.2`, but the upstream Leo source tag for this release is `leo-lang-v4.4.2`. Build scripts and workflows keep these separate:

- `LEO_VERSION=v4.4.2` controls the public image tag.
- `LEO_SOURCE_TAG=leo-lang-v4.4.2` controls the upstream git clone and Rust toolchain inference.
...
- `SNARKOS_VERSION=v4.9.1` controls the public image component (the `vLEO-vSNARKOS` tag and the `snarkos.version` label).
- `SNARKOS_SOURCE_TAG=v4.9.1` controls the upstream git clone and Rust toolchain inference (recorded via the `snarkos.source-tag` label).

This is needed for releases that upstream publishes under a non-`vX.Y.Z` tag: `v4.8.1` is published from the pre-release tag `testnet-v4.8.1`. When `SNARKOS_SOURCE_TAG` is empty, `SNARKOS_VERSION=v4.8.1` derives `testnet-v4.8.1`; other snarkOS versions (e.g. the default `v4.9.1`) fall back to using `SNARKOS_VERSION` as the source tag.

The current default pair is `v4.4.2-v4.9.1`. Leo `leo-lang-v4.4.2` and snarkOS `v4.9.1` both pin snarkVM `4.9.1` from crates.io. snarkVM `4.9.1` defines consensus versions up to V19, so devnets and deployment snapshots on this pair use 19 consensus heights (`0..18`).
...
./build-publish-deployment-snapshot.sh --version v4.4.2-v4.9.1 --skip-push

# Build and push to registry (requires authentication)
./build-publish-deployment-snapshot.sh --commit main --version v4.4.2-v4.9.1
...
# Produces: leo-lang:v4.4.2-node24, leo-lang:latest
...
# Produces: aleo-devnet:v4.4.2-v4.9.1-rust188, aleo-devnet:latest
```
...
|---|---|---|
| `LEO_VERSION` | both | Normalized Leo image/version tag (default: `v4.4.2`) |
| `LEO_SOURCE_TAG` | leo-lang | Upstream Leo source tag used for clone/toolchain inference (default: `leo-lang-v4.4.2` when `LEO_VERSION=v4.4.2`) |
| `SNARKOS_VERSION` | aleo-devnet | Normalized snarkOS image/version tag (default: `v4.9.1`) |
| `SNARKOS_SOURCE_TAG` | aleo-devnet | Upstream snarkOS source tag used for clone/toolchain inference (default: `SNARKOS_VERSION`; `testnet-v4.8.1` when `SNARKOS_VERSION=v4.8.1`) |
...
LEO_VERSION="v4.4.2" LEO_SOURCE_TAG="leo-lang-v4.4.2" LEO_REPO="https://github.com/your-fork/leo" NODE_VERSION=18 \
  ./build-publish-image.sh --dockerfile leo.Dockerfile --image-name leo-lang
```
...
```bash
docker run --rm -v $(pwd):/app -w /app --user $(id -u):$(id -g) ghcr.io/sealance-io/leo-lang:v4.4.2 leo build
```
