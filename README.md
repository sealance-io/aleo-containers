...
- **Leo Lang**: `ghcr.io/sealance-io/leo-lang:v4.2.0`
- **Aleo Devnet**: `ghcr.io/sealance-io/aleo-devnet:v4.2.0-v4.7.3`
...
docker run --rm ghcr.io/sealance-io/leo-lang:v4.2.0

# Mount your project directory and work with Leo
docker run --rm -v $(pwd):/app -w /app ghcr.io/sealance-io/leo-lang:v4.2.0 leo build

# Start a shell in the container
docker run --rm -it -v $(pwd):/app -w /app ghcr.io/sealance-io/leo-lang:v4.2.0 /bin/bash
```
...
docker run -it --rm -p 3030:3030 -p 4130:4130 \
  ...
  ghcr.io/sealance-io/aleo-devnet:v4.2.0-v4.7.3
...
Published image tags stay normalized as `v4.2.0`, but the upstream Leo source tag for this release is `leo-lang-v4.2.0`. Build scripts and workflows keep these separate:

- `LEO_VERSION=v4.2.0` controls the public image tag.
- `LEO_SOURCE_TAG=leo-lang-v4.2.0` controls the upstream git clone and Rust toolchain inference.

When `LEO_SOURCE_TAG` is empty, the default `LEO_VERSION=v4.2.0` derives `leo-lang-v4.2.0`; other Leo versions fall back to using `LEO_VERSION` as the source tag for compatibility.
...
./build-publish-deployment-snapshot.sh --version v4.2.0-v4.7.3 --skip-push

# Build and push to registry (requires authentication)
./build-publish-deployment-snapshot.sh --commit main --version v4.2.0-v4.7.3
...
# Produces: leo-lang:v4.2.0-node24, leo-lang:latest
...
# Produces: aleo-devnet:v4.2.0-v4.7.3-rust188, aleo-devnet:latest
```
...
|---|---|---|
| `LEO_VERSION` | both | Normalized Leo image/version tag (default: `v4.2.0`) |
| `LEO_SOURCE_TAG` | leo-lang | Upstream Leo source tag used for clone/toolchain inference (default: `leo-lang-v4.2.0` when `LEO_VERSION=v4.2.0`) |
| `SNARKOS_VERSION` | aleo-devnet | snarkOS release tag (default: `v4.7.3`) |
...
LEO_VERSION="v4.2.0" LEO_SOURCE_TAG="leo-lang-v4.2.0" LEO_REPO="https://github.com/your-fork/leo" NODE_VERSION=18 \
  ./build-publish-image.sh --dockerfile leo.Dockerfile --image-name leo-lang
```
...
```bash
docker run --rm -v $(pwd):/app -w /app --user $(id -u):$(id -g) ghcr.io/sealance-io/leo-lang:v4.2.0 leo build
```
