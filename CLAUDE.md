...
# Build deployment snapshot (default base v4.4.2-v4.9.1, consensus 19)
...
`RUST_VERSION` is auto-inferred — only override explicitly. Keep Rust per component: Leo `v4.4.2` uses Rust `1.96.0`; snarkOS `v4.9.1` declares Rust `1.88`, normalized to Docker base tag `1.88.0`.

### Leo Source Tags vs Image Tags

Public Leo image tags are normalized (`LEO_VERSION=v4.4.2`), but upstream source uses `LEO_SOURCE_TAG=leo-lang-v4.4.2`. `LEO_VERSION` controls published image tags; `LEO_SOURCE_TAG` controls the Leo git clone and Rust inference. If unset, known Leo releases with `leo-lang-*` upstream tags derive the matching source tag automatically; other versions fall back to `LEO_VERSION`.

### snarkOS Source Tags vs Image Tags

Same split as Leo. `SNARKOS_VERSION` (e.g. `v4.9.1`) is the normalized image component used in `vLEO-vSNARKOS` tags and the `snarkos.version` label; `SNARKOS_SOURCE_TAG` controls the snarkOS git clone and Rust inference and is recorded via the `snarkos.source-tag` label. Needed because `v4.8.1` is published from the pre-release tag `testnet-v4.8.1`. If unset, only `SNARKOS_VERSION=v4.8.1` derives `testnet-v4.8.1`; other versions (e.g. the default `v4.9.1`) pass through unchanged.
